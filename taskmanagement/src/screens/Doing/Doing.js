...
const Doing = ({navigation}) => {
  ...
  if (dataGql) {
    console.log(
      dataGql.findAllTaskWorker.filter(f => f.status == 'doing').length == 0,
    );
  }
  return (
    <View style={styles.container}>
      ...
      {!dataGql && !time ? (
      ...
      ) : dataGql.findAllTaskWorker.filter(f => f.status == 'doing').length ==
        0 ? (
        ...
        <FlatList
          data={dataGql.findAllTaskWorker.filter(f => f.status == 'doing')}
          renderItem={RCard}
